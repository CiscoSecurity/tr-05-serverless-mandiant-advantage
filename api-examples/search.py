...
class MandiantClient:
	...
	def do_token(self):
		...
		access_token = requests.post(
			url=self._TOKEN_URL, headers=self.headers, data=params).json().get("access_token")
	...
	def actor_enrich(self, id):
		...
		resp = requests.get(url=url, headers=self.headers, data=json.dumps(params))
	...
	def do_search(self, search_text, limit=1000, next_url=None):
		if len(self.objects) >= self.ctr_entities_limit: return self.objects
		...
		resp = requests.post(url=url, headers=self.headers, data=json.dumps(params))
				...
				for object in objects:
					...
					if 'actors' in object:
						...
						for actor in object['actors']:
							actor_id = actor['id']
							actor_enrichment.append(self.actor_enrich(actor_id))
					...
					if 'malwares' in object:
						...
						for malware in object['malwares']:
							...
							malware_enrichment.append(self.malware_enrich(malware_id))
                   
						object['malwares'] = list(filter(None, malware_enrichment))
					if len(self.objects) >= self.ctr_entities_limit: return self.objects
