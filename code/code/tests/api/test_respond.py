...
from code.tests.payloads_for_tests import EXPECTED_RESPONSE_OF_JWKS_ENDPOINT
...
def test_respond_call_with_valid_jwt_but_invalid_json_action_id(
    ...
    response = client.post(
        route, headers=get_headers(valid_jwt()), json=invalid_json_action_id
    )
    assert response.status_code == HTTPStatus.OK
    assert response.json == invalid_json_expected_payload(
