...
class Mapping:
    ...
    def observed_time(self, event):
        ...
        return {"start_time": event_time, "end_time": event_time}
    ...
    def get_relations(event):
        if (event.get("ip_src") and event.get("ip_dst")) and (
            ...
            return [
                {
                    "related": {"type": "ip", "value": event["ip_dst"]},
                    "source": {"type": "ip", "value": event["ip_src"]},
    ...
    def targets(self, event):
        ...
        if event.get("ip_dst"):
            observables.append({"type": "ip", "value": event["ip_dst"]})
        if event.get("filename"):
            observables.append(
                {"type": "file_name", "value": event["filename"]}
            )
        if event.get("eth_dst"):
            observables.append(
                {"type": "mac_address", "value": event["eth_dst"]}
            )
        if event.get("username"):
            observables.append({"type": "user", "value": event["username"]})
    ...
    def sighting(self, observable, event):
        ...
        d = {
            ...
            "short_description": f"RSA Netwitness session ID {event['sessionid']} ({event['netname']})",
